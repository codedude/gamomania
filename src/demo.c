...
// 	glUseProgram(appData->program->id);
// 	Material *container = Material_createTexture(
// 	    "../model/backpack/diffuse.jpg", "../model/backpack/specular.jpg",
// 	    (vec3){0.2f, 0.2f, 0.2f}, 64.0f);
...
// 	Model *model = Model_create("backpack/backpack.obj");
// 	appData->model = model;
...
static bool loadVertices(AppData *appData) {
	glUseProgram(appData->program->id);
	...
	appData->model = Model_create("backpack/backpack.obj");
	CHECK_ALLOC(appData->model, false);
...
bool demoSetup(AppData *appData) {
	...
	glBindTexture(GL_TEXTURE_2D, appData->program->container->texture.diffuse);
	Shader_setInt(
	    Shader_uniformGet(appData->program, "material.texture_diffuse1"), 0);
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, appData->program->container->texture.specular);
	Shader_setInt(
	    Shader_uniformGet(appData->program, "material.texture_specular1"), 1);
	Shader_setFloat(
	    Shader_uniformGet(appData->program, "material.texture_shininess"),
	    64.0f);
